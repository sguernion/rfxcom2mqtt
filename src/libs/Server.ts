...
import StateStore, {DeviceStore} from './state';
...
export default class Server {
    ...
    constructor(conf: Settings,devices: DeviceStore,state: StateStore,bridgeInfo: BridgeInfo) {
        ...
        this.router.get('/', (req: Request, res: Response) => {
         ...
         this.router.get('/settings', (req: Request, res: Response) => {
            res.header("Access-Control-Allow-Origin", "*").status(StatusCodes.OK).json(conf);
         });

         this.router.get('/devices', (req: Request, res: Response) => {
            res.header("Access-Control-Allow-Origin", "*").status(StatusCodes.OK).json(devices.getAll());
         });
         
         this.router.get('/devices/status', (req: Request, res: Response) => {
            res.header("Access-Control-Allow-Origin", "*").status(StatusCodes.OK).json(state.getAll());
         });

         this.router.get('/bridge/info', (req: Request, res: Response) => {
            res.header("Access-Control-Allow-Origin", "*").status(StatusCodes.OK).json(this.bridgeInfo);
         });
    ...
    private isHttpsConfigured():boolean {
        if (this.frontConf.sslCert && this.frontConf.sslKey) {
            if (!fs.existsSync(this.frontConf.sslCert) || !fs.existsSync(this.frontConf.sslKey)) {
                logger.error(`defined ssl_cert '${this.frontConf.sslCert}' or ssl_key '${this.frontConf.sslKey}' file path does not exists, server won't be secured.`); /* eslint-disable-line max-len */
                return false;
            }
            return true;
        }
    ...
    async start(): Promise<void> {
        ...
        if (!this.frontConf.host) {
            ...
            this.server.listen(this.frontConf.port, this.frontConf.host,() => {
                logger.info(`Started frontend on port ${this.frontConf.host}:${this.frontConf.port}`);
            });
        }
